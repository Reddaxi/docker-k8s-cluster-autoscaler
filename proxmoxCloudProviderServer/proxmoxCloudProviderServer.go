...
// ProxmoxCloudProviderServer is a placeholder for your implementation.
// Add your functions and types here.
import (
	"context"
	"fmt"
	"log"
...
var nodeGroupTargetSizes = map[string]int32{
	"proxmox-virtual-node-group": 1, // Start with 1 node as default
}
...
func (c *CloudProviderServer) NodeGroupIncreaseSize(context context.Context, r *protos.NodeGroupIncreaseSizeRequest) (*protos.NodeGroupIncreaseSizeResponse, error) {
	...
	for i := 0; i < int(r.Delta); i++ {
		containerProxmoxID, err := c.proxmoxClient.CreateContainer(fmt.Sprintf("%s-%d", r.Id, i))
		if err != nil {
			return nil, err
		}
		log.Printf("Created container '%s' for node group '%s'", containerProxmoxID, r.Id)
		addProxmoxContainerAsNode(c, containerProxmoxID, r.Id)
	}
...
func (c *CloudProviderServer) NodeGroupDeleteNodes(context context.Context, r *protos.NodeGroupDeleteNodesRequest) (*protos.NodeGroupDeleteNodesResponse, error) {
	...
	for _, node := range r.Nodes {
		...
		err := c.proxmoxClient.DeleteContainer(node.Name)
		if err != nil {
...
func addProxmoxContainerAsNode(c *CloudProviderServer, containerProxmoxID string, nodeGroupID string) {
	c.instances = append(c.instances, &protos.Instance{
		Id:     containerProxmoxID,
		Status: &protos.InstanceStatus{InstanceState: protos.InstanceStatus_instanceRunning},
	})
	log.Printf("Added container '%s' as node to node group '%s'. There are now a total of '%d' nodes.", containerProxmoxID, nodeGroupID, len(c.instances))
}
